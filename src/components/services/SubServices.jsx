...
export default function Subservices({subservices, heading}) {
  ...
  return (
      <section className=" py-14 sm:py-16 lg:py-20 bg-[#0b1320] overflow-x-clip">
      {/* Heading + Subheading */}
      <motion.div              
                initial={{ opacity: 0, y: 50 }}
                whileInView={{ opacity: 1, y: 0 }}
  ...
  Explore our subservices below to see how we tailor solutions for your business needs.
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
        ...
        {/* Left: Subservice List */}
         <div className="lg:col-span-4">
         <div className="lg:hidden">
        <div className="  flex gap-3 overflow-x-auto no-scrollbar pb-2 -mx-1 px-1 snap-x snap-mandatory scroll-pt-2 sticky top-2 z-10 bg-[#0b1320] max-[640px]:gap-2 max-[640px]:pb-1 max-[640px]:-mx-0 " 
        >
          {Object.keys(subservices).map((service) => (
            <button   
              key={service}
              onClick={(e) => { 
                setActive(service);
                /* ADDED: center selected chip on phones */
                try { e.currentTarget.scrollIntoView({ inline: 'center', block: 'nearest', behavior: 'smooth' }); } catch {}
              }}
              className={`w-full whitespace-nowrap px-4 py-2 rounded-2xl text-sm transition transform text-left p-4 min-[1270px]:text-xl max-[860px]:text-sm text-md font-large font-semibold
                ${
                  active === service
                ...
                w-auto shrink-0 
                max-[640px]:px-2.5 max-[640px]:py-1.5 max-[640px]:text-[11px] max-[640px]:rounded-xl 
                max-[640px]:leading-none max-[640px]:tracking-tight max-[640px]:h-8 max-[640px]:min-w-[88px]
                max-[640px]:w-auto max-[640px]:shrink-0 max-[640px]:scale-100`} 
            >
              {service}
            </button>
          ))}
        ...
        {/* Desktop: vertical list */}
            ...
            {/* Right: Active Content */}
            <div className="relative w-5xl lg:w-4xl bg-blue-300 rounded-4xl shadow-lg p-10 
                            max-[1023px]:w-full max-[1023px]:p-5 max-[640px]:p-4"  /* ADDED fit + padding for mobile */> 
              <AnimatePresence mode="wait"> <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }} > 
                <h3 className="text-4xl font-bold px-5 font-large text-gray-800 mb-4 mt-4 
                               max-[640px]:text-3xl max-[420px]:text-2xl"  /* ADDED smaller title on phones */> {active} </h3>
                 <div className="space-y-4 p-8 max-[1023px]:p-5 max-[640px]:p-4"  /* ADDED responsive inner padding */> {subservices[active].desc.map((point, idx) => 
                  ( <div key={idx} className="flex items-start  gap-2"> 
                  <CheckCircle className="text-blue-500 flex-shrink-0 mt-1" size={22} /> 
                  <p className="text-gray-900 text-lg leading-relaxed max-[640px]:text-base"  /* ADDED readable on phones */> {point} </p> </div> ))} 
                  </div> </motion.div> </AnimatePresence> 
                  </div>
        </div>
        </motion.div>
